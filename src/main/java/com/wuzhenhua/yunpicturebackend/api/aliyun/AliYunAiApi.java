package com.wuzhenhua.yunpicturebackend.api.aliyun;
...
public class AliYunAiApi {
    ...
    public CreateOutPaintingTaskResponse createOutPaintingTask(CreateOutPaintingTaskRequest createOutPaintingTaskRequest) {
        ...
        // Executes request; returns response or throws exception
        try (HttpResponse httpResponse = httpRequest.execute()) {
            if (!httpResponse.isOk()) {
                log.error("阿里云扩图任务创建失败: {}", httpResponse.body());
                throw new BusinessException(ErrorCode.OPERATION_ERROR, "调用阿里云AI扩图接口失败");
            }
            ...
            if (createOutPaintingTaskResponse.getCode() != null) {
                ...
                log.error("阿里云扩图任务创建失败: {}", errorMessage);
                throw new BusinessException(ErrorCode.OPERATION_ERROR, "阿里云扩图任务创建失败");
            }
