...
create index chat_history_session_id_index on chat_history (session_id);

create index idx_create_time on chat_history (create_time);

create index idx_picture_id on chat_history (picture_id);

create index idx_user_id on chat_history (user_id);

-- 对话历史图片关联表
create table if not exists chat_history_picture
(
    id              bigint auto_increment comment 'id' primary key,
    chat_history_id bigint                             not null comment '对话历史ID',
...
) comment '对话历史图片关联表' collate = utf8mb4_unicode_ci;
